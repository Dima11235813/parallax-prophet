...
export interface Component<T> { readonly name: string; defaults(): T }
...
export interface World {
  ...
  query<Q extends Component<any>[]>(...all: Q): Iterable<[Entity, ...{ [K in keyof Q]: ReturnType<Q[K]['defaults']> }]>;
}

export interface SystemCtx { world: World; time: Time; events: EventBus; rng: RNG }
...
export function run(main: Main) {
  ...
  function frame(now: number) {
    const clamped = Math.min(0.25, (now - last) / 1000); last = now; acc += clamped;
    while (acc >= dt) { main.update(dt); acc -= dt; }
    main.render(acc / dt);
...
export interface PerspectiveAnchor { id: string; position: [number, number, number] }
...
export const anchors = [
  { id: 'door-socket', position: [10, 2, -5] },
] as const;
